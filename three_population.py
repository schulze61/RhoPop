...
number_hyper_params = 8
    
def ptform(u):
    x = np.array(u)  # copy u
        
    #Hyperparameter prior transformation
    #x[0], x[2], x[4] are the mean compositions of the highest, moderate, and lowest density populations, respectively.
    #x[1], x[3], x[5] are the respective intrinsic scatter parameters        
    x[0] = 0.94*u[0]
    x[1] = 2.0*u[1]
    x[2] = x[0]*u[2]
    x[3] = 2.0*u[3]
...
def loglike(x):
        ...
        if x[4] >= 0.29:
            iline_c3 = f.grids.build_isoline_rock(x[4])/f.grids.build_isoline_rock(0.29)
        else:
            wmf = f.grids.cmf_to_wat(x[4])
            iline_c3 = f.grids.build_isoline_wat(wmf)/f.grids.build_isoline_rock(0.29)
        ...
        ii3 = si.interp1d(f.grids.get_mass_array(), iline_c3, fill_value = 'extrapolate')
    
        sigk1 = np.sqrt(data_err[:,1]**2 + x[1]**2)
        sigk2 = np.sqrt(data_err[:,1]**2 + x[3]**2)
        sigk3 = np.sqrt(data_err[:,1]**2 + x[5]**2)

        pp1 = sp.norm.pdf(data[:,1], loc = ii1(X_tmp), scale = sigk1)
        pp2 = sp.norm.pdf(data[:,1], loc = ii2(X_tmp), scale = sigk2)
        pp3 = sp.norm.pdf(data[:,1], loc = ii3(X_tmp), scale = sigk3)
        
        
        if len(np.where(np.isnan(pp1) == True)[0]) > 0 or len(np.where(pp1 == 0)[0]) > 0:
            return -np.inf
        elif len(np.where(np.isnan(pp2) == True)[0]) > 0 or len(np.where(pp2 == 0)[0]) > 0:
            return -np.inf
        elif len(np.where(np.isnan(pp3) == True)[0]) > 0 or len(np.where(pp3 == 0)[0]) > 0:
            return -np.inf  
        elif x[6] + x[7] > 1.0:
            return -np.inf
        else:
            pp = sum(np.log(x[6]*pp1 + x[7]*pp2 + (1.0-x[6]-x[7])*pp3))
...
colnames[0:number_hyper_params] = ['c1', 'sc1', 'c2', 'sc2', 'c3', 'sc3', 'mw1', 'mw2']
...
if __name__ == '__main__':
    with dynesty.pool.Pool(cpu_count(), loglike, ptform) as pool:
        ...
        if dynesty_plots == True:
            dynesty_plotting.three_population.runplot(results, froot = dfile_root)
            dynesty_plotting.three_population.corner(results, froot = dfile_root)
            dynesty_plotting.three_population.traceplot(results, froot = dfile_root)
